...
**（1）组件模板**（每个重复项包含哪些子元素，按视觉位置从上到下排列）

| 元素位置 | 元素名称 | 类型 | 内容模式 | 来源 |
|----------|----------|------|----------|------|
| 行1左 | [如：业务标题] | 文本 | [内容模式：如 [业务类型] · [主体名称]] | 原型明确 |
| 行1右 | [如：状态标签] | 标签/纯文本 | [见下方状态枚举表] | 原型明确 |
| 行2 | [如：说明文案] | 文本 | [句式模式：如 [平台动作]；[用户下一步]] | 原型明确 |
| 行3 | [如：辅助信息] | 文本 | [编号/时间/关联对象] | 原型明确 |
| 右侧 | [如：操作按钮] | 按钮/链接 | [见下方状态×操作映射表] | 原型明确 |
...
11. **复合组件拆解检查**：页面含列表、卡片组、表格、标签页等复合组件时，是否有 3.X.2 拆解（组件模板/状态枚举/状态×操作映射）？
...
- **复合组件必须拆解**：列表、卡片组、表格、标签页等复合组件不得在 UI 元素清单里概括为单行，必须按 3.X.2 的七张子表拆解（组件模板/状态枚举/状态×操作映射/分组排序/数据状态/数量边界/共存移动）
